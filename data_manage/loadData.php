...
// 				 'SPAIN' => array('PRIMERA+DIVISION'));
	...
	// /$leagues = array('CYPRUS' => array('DIVISION+1'));

	foreach ($seasons as $season) {

		$seasonName = str_replace("%2F", "-", $season);
		$url = "http://www.xscores.com/soccer/Results.jsp?sport=1&countryName=CYPRUS&leagueName=DIVISION+1&seasonName=$season&sortBy=R&result=3#.UvjQvkKSztk";
		echo "$url";
		...
		$firstRound = 111052;//$select->firstChild->getAttribute('value');
		$roundCount = 26;//$select->lastChild->nodeValue;
		...
		for ($j = 0; $j < $roundCount; $j ++) {
			...
			$url = "http://www.xscores.com/soccer/Results.jsp?sport=1&countryName=CYPRUS&leagueName=DIVISION+1&seasonName=$season&sortBy=R&round=$n&result=3#.UvjQ30KSztk";
			echo "$url<br>";
