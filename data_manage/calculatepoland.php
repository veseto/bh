...
// 	$mysqli -> query("INSERT INTO series (team, length, active, leagueId) values ('POLAND', 1, 1, 18)");

// 	$q = "SELECT * from matches where leagueId=18 and season='2013-2014' and state<>'Post' order by matchDate asc, matchTime asc";
// 	$mRes = $mysqli->query($q);
// 	$previousHasResult=False;
// 	while ($match = $mRes->fetch_assoc()) {
// 		$seriesId = $mysqli->query("select seriesId from series where team='POLAND' and active=1")->fetch_array()[0];
